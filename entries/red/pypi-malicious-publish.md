...
Every downstream `pip install` — CI runners, developer laptops, images — resolves and runs
your code (a malicious `setup.py`/build hook runs at install time), so one upload fans out
across the dependency graph. A token upload also bypasses the project's OIDC **trusted
